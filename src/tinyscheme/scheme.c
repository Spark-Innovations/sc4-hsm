...
unsigned char readc1(void) { return getchar(); }
void unreadc(unsigned char c) { ungetc(c, stdin); }
#undef USE_STRLWR
#define USE_STRLWR 1
#endif
...
unsigned char readc1(void) {
  if (unreadchar > 0) {
    unsigned char c = unreadchar & 0xFF;
    unreadchar = -1;
    return c;
  } else {
    return readc();
...
void unreadc(unsigned char c) { unreadchar = c; }
#endif
...
static INLINE void ok_to_freely_gc(scheme *sc) { car(sc->sink) = sc->NIL; }
...
static int inchar(scheme *sc) {
  ...
  return (int)readc1() & 0xff;

  int c;
  port *pt;

  pt = sc->inport->_object._port;
  if (pt->kind & port_saw_EOF) return EOF;
  c = basic_inchar(pt);
...
#else
      // printf of floats is broken in gcc-arm
      float f = rvalue_unchecked(l);
      int intpart = (int)f;
      int fracpart = (int)((f-floor(f))*1e6);
      snprintf(p, STRBUFFSIZE, "%d.%06d", intpart, fracpart);
#endif
    }
  } else if (is_string(l)) {
    if (!f) {
...
static int is_nonneg(pointer p) { return is_integer(p) && ivalue(p) >= 0; }
...
static void Eval_Cycle(scheme *sc, enum scheme_opcodes op) {
  ...
  for (;;) {
    ...
    if (pcd->name != 0) { /* if built-in function, check arguments */
      ...
      if (n < pcd->min_arity) {
        ok = 0;
        snprintf(msg, STRBUFFSIZE, "%s: needs%s %d argument(s)", pcd->name,
      ...
      if (ok) {
        if (pcd->arg_tests_encoding != 0) {
          ...
          if (i < n) {
            ok = 0;
            snprintf(msg, STRBUFFSIZE, "%s: argument %d must be: %s", pcd->name,
                     i + 1, tests[j].kind);
          }
        }
      }
    ...
    ok_to_freely_gc(sc);
    if (pcd->func(sc, (enum scheme_opcodes)sc->op) == sc->NIL) return;
    if (sc->no_memory) {
      print("Out of memory\n");
      return;
    }
...
static struct scheme_interface vtbl = {
    scheme_define, s_cons, s_immutable_cons, reserve_cells, mk_integer, mk_real,
    mk_symbol, gensym, mk_string, mk_counted_string, mk_character, mk_vector,
    mk_foreign_func, putstr, putcharacter,

    is_string, string_value, is_number, nvalue, ivalue, rvalue, is_integer,
    is_real, is_character, charvalue, is_list, is_vector, list_length, ivalue,
    fill_vector, vector_elem, set_vector_elem, is_port, is_pair, pair_car,
    pair_cdr, set_car, set_cdr,

    is_symbol, symname,

    is_syntax, is_proc, is_foreign, syntaxname, is_closure, is_macro,
    closure_code, closure_env,

    is_continuation, is_promise, is_environment, is_immutable, setimmutable,
...
int scheme_init_custom_alloc(scheme *sc, func_alloc malloc, func_dealloc free) {
  ...
  int cnt = alloc_cellseg(sc, FIRST_CELLSEGS);
  if (cnt != FIRST_CELLSEGS) {
    sc->no_memory = 1;
    print("Allocated only %d cellsegs\n", cnt);
    return 0;
  }
  sc->gc_verbose = 0;
...
void save_from_C_call(scheme *sc) {
  pointer saved_data = cons(sc, car(sc->sink), cons(sc, sc->envir, sc->dump));
  /* Push */
...
/*
int _EXFUN(fprintf, (FILE * __restrict f, const char *__restrict s, ...)) {
  print("fprintf: %s\n", s);
  return 0;
}
*/
...
pointer _abs(scheme *sc, pointer args) {
  if(args!=sc->NIL) {
    pointer p =pair_car(args);
    if (is_number(p)) {
      if (is_integer(p)) {
	return mk_integer(sc, abs(ivalue(p)));
      }else {
	return mk_real(sc, fabs(rvalue(p)));
      } 
    }
  }
  return sc->NIL;
...
pointer _moire(scheme *sc, pointer args) {
  moire();
  return sc->NIL;
}

void lcd_print(char*);
pointer _lprint(scheme *sc, pointer args) {
  if(args!=sc->NIL) {
    if(is_string(pair_car(args))) {
      char *s = strvalue(pair_car(args));
      lcd_print(s);
    }
  }
  return sc->NIL;
...
void lcd_noise();
pointer _noise(scheme *sc, pointer args) {
  lcd_noise();
  return sc->NIL;
}
...
#define SDEF(name, entry) \
  scheme_define(&sc, sc.global_env, mk_symbol(&sc, name), \
		mk_foreign_func(&sc, entry))
