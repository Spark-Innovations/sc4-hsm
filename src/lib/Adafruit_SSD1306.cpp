...
void inline rstn(uint8_t value) {
  HAL_GPIO_WritePin(DISP_PORT, DISP_RSTn_PIN,
                    value ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

void inline csn(uint8_t value) {
  HAL_GPIO_WritePin(DISP_PORT, DISP_CSn_PIN,
                    value ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

void inline dcn(uint8_t value) {
  HAL_GPIO_WritePin(DISP_PORT, DISP_DCn_PIN,
                    value ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
...
#if 0
  // our hardware is already set up by main(); the SPI is initialized and the control lines are all in the default state.
  _i2caddr = i2caddr;
